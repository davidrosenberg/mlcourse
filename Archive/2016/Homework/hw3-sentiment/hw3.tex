...
in each epoch. Show that if we use a fixed step size $1$ and we terminate
when our training loss is $0$, then we are exactly doing the Perceptron
algorithm.
...
Investigate how often and when we have $y_{i}w^{T}x_{i}=1$. Describe
your findings. If we didn't know about subgradients, one might suggest
just skipping the update when $yw^{T}x_{i}=1$. Does this seem reasonable? 
\end{enumerate}
...
along with $x_{i},w_{i},x_{i}w_{i}$. Do you understand why the model
