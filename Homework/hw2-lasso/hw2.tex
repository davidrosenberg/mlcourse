...
which occurs when the $j$th column of $X$ is the identically $0$.
One can either eliminate the column (as it cannot possibly help the
solution), or you can set $w_{j}=0$ in that case, as that is, in
fact, the coordinate minimizer in that case.Note that Murphy is suggesting
to initialize the optimization with the ridge regession solution,
