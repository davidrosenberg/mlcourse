...
Which of the following functions can be exactly represented by this neural network?
\begin{itemize}
\item polynomials of degree one: $l(x) = ax+b$ \textbf{Yes}
...
\begin{itemize}
\item \begin{align*}
\frac{\partial \mathcal{L}}{\partial U} &= \sum_{t=1}^T \left(\Pi_{k=0}^{t-1} (\bU^T D_k) \right ) \bh_{t-1}^T\\
\frac{\partial \mathcal{L}}{\partial W} &= \sum_{t=1}^T \left(\Pi_{k=0}^{t-1} (\bU^T D_k) \right ) \bx_{t-1}^T\
\end{align*}
$D_{k-1} = \text{diag}(g'(\bz_{t-1}))$ is the Jacobian matrix of the element-wise activation function.
