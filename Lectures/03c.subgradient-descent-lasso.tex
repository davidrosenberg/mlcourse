...
\begin{itemize}
\item The gradient of the Lasso objective is
\[
\del_{w}J(w)=\frac{1}{n}\sum_{i=1}^n 2\{w^Tx_i-y_i\}x_i+\lambda\sum_{i=1}^d\text{sign}(w_i)
\]
...
\end{block}
Question: Is the $\le$ in the constraint just a convention? Could
we also have used $\ge$ or $=$?
\end{frame}
%
...
\begin{center}
\includegraphics[height=0.75\textheight]{../Figures/subgradient-methods/subdiff-contour-plot-abs-x1-plus-2absx2}
\par\end{center}
...
\end{frame}
%
\begin{frame}{Contour Lines and Gradients}
\begin{itemize}
\item For function $f:\reals^{d}\to\reals$, 
\begin{itemize}
\item \textbf{graph} of function lives in $\pause\reals^{d+1}$,

\pause{}
\item \textbf{gradient} and \textbf{subgradient} of $f$ live in $\pause\reals^{d}$,
and

\pause{}
\item \textbf{contours}, \textbf{level sets,} and \textbf{sublevel sets}
are in $\reals^{d}$.
\end{itemize}
\end{itemize}
...
\pause{}
\begin{itemize}
\item $f:\reals^{d}\to\reals$ continuously differentiable, $\del f(x_{0})\neq0$,
then $\del f(x_{0})$ normal to level set
\[
...
\pause{}
\item Proof sketch in notes.
\end{itemize}
\end{frame}
%
\begin{frame}{Gradient orthogonal to sublevel sets}
\begin{center}
\includegraphics[height=0.75\textheight]{../Figures/subgradient-methods/grad-orthog-to-sublevel-sets}\let\thefootnote\relax\footnotetext{\tiny{Plot courtesy of Brett Bernstein.}}
\par\end{center}
\end{frame}
...
\begin{frame}{Subgradient Gets Us Closer To Minimizer}
\begin{theorem}
Suppose $f$ is convex.
\begin{itemize}
...
\end{itemize}
$\implies$\textbf{Negative subgradient step gets us closer to minimizer}.
\end{frame}
...
\[
\frac{1}{n}\sum_{i=1}^n 2\{w^Tx_i-y_i\}x_i+\lambda\sum_{i=1}^ds_i,
\]
...
\begin{itemize}
\item Subgradient descent has very poor convergence properties for non-smooth functions	(Like Lasso)
\item Subgradient descent ignores problem structure
\item Taking small steps in the direction of the (sub)gradient usually may not lead to zero coordinates
\item Use Shooting Method or Projected SGD instead!
\end{itemize}
